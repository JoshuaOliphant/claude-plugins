...
1. **Find the gates.** Read the gates `compost:setup` recorded in `docs/agents/testing.md`: the
   test command, the coverage threshold and any recorded exclusions, linters, and type checkers. If none are recorded, the
   project's CI workflow defines green: run what `.github/workflows/*` runs, plus any
   `pre-commit`, `Makefile` or `justfile` check target, and suggest `compost:setup`.
...
2. **Run the full suite and every gate, fresh.** Run them in this worktree now, not earlier and
   ...
   and later failures are often its echoes. Fix it if the cause is plain; otherwise hand it to
   `compost:diagnose`. A warning the gate prints is a finding too: fix it, or say why it stays.
...
3. **Hold the coverage gate.** The gate is 100% line coverage, for example
   ...
   - Covering it would take a test that proves nothing (asserting a constant, re-testing the
     ...
     line) or a lower threshold. Do not pad the suite to reach the number.

4. **Map every acceptance criterion to a passing test.** Build a table from this run's results:
   AC-N, the test's node id, and pass or fail. Find the tests by the names and parametrize ids
   `compost:build` gave them. A criterion with no test is unmet, with no judgment call. So is one
...
5. **Prove the one fact the change is safe because of.** Most changes that look risky are safe
   ...
   4. Ran: a script or test calls the real code and fails loudly if the fact is false. Claude
      Code's bundled `/verify` can build and exercise the change for you.
...
7. **Check the comments in the diff.** For Python, run `jev-lint --diff <base>` when the jev-lint
   plugin is installed. Exit 3 means `TYPESAFE_API_KEY` is not set; say so and do the manual
   pass. Otherwise read every comment the diff adds. Keep only:
...
8. **Report the evidence.** Give the commands and their counts (`214 passed, 0 failed`), the
   ...
   the proof output, and the comments removed. Post the AC table on the issue. Say "done" only
