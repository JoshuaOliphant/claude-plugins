...
1. **Run the review workflow.** Run `/compost:review-changes` with `{base, head, issue, jev: "${CLAUDE_PLUGIN_ROOT}/scripts/jev.py"}`. Leave `base` out to review against the merge-base with the default branch. Leave `head` out to review up to `HEAD`; pass it to review one issue's commits on a branch that has moved on, such as `<merge>^2` for an issue landed by a merge. Pass `issue` when you know the issue number, since the Spec axis needs its AC-N. The workflow scopes the diff and ranks the changed files by structural risk with Jev's `review-risk`, so reviewers read the riskiest first. It runs a Standards reviewer and a Spec reviewer as `compost:reviewer` subagents in parallel; each picks its canon essays, anchors its findings, and grades their severity with Jev ([Jev tools](references/jev-tools.md)). A skeptic told to refute it gets each finding Jev marks worth one: a probable blocker or major, or a call Jev is unsure of. The rest come back marked unverified. When Jev is unavailable, the reviewers grade severity themselves and every blocker and major gets a skeptic, while minor findings skip it. Pass `skeptics: 3` for a branch where a false finding would be expensive; a finding then survives only when most skeptics uphold it.

   Running the workflow from this skill is the opt-in it needs: it spawns a handful of agents, not dozens. If the Workflow tool is unavailable, as inside a subagent, do the same by hand: run `review-risk` yourself (input `{"base": ..., "head": ...}`), spawn two `compost:reviewer` agents in one message, one on the Standards axis and one on the Spec axis, with the ranked file list in each prompt, then one skeptic pass over the findings whose `skeptic` is true, or over every blocker and major when the reviewers left `skeptic` out.
   ...
   Each axis also returns an `unjudged` list: findings sent to a skeptic whose skeptics all failed, so nobody tried to refute them. Treat every unjudged finding as a survivor in the steps below.

   The result's `risk` holds the ranked files and a `structural` flag. When `risk.ranked` is false, `risk.problem` says why (no Jev path, Jev unavailable), and structural risk is yours to judge in the section below.
...
The trigger is the workflow's `risk.structural`: run approach fit when it is true, and keep the flagged file names out of the problem statement, since they hint at the mechanism. When `risk.ranked` is false, decide from the list above by reading the changed files. The flag misses about one structural file in seven, so a branch you can see adds one of those things still gets the judge.
