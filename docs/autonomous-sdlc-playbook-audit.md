...
event as "Replaces default git behavior" and the contract as printing the new
worktree's absolute directory path on stdout (`WorktreeRemove` mirrors it for
cleanup); any non-zero exit aborts creation. A logging-only hook there
...
| 5 | Journal classifier denials instead of stalling on them | `PermissionDenied` fires when auto mode denies a call; `hookSpecificOutput.retry` tells the model whether it may retry | Pairs with moving the Builder from `bypassPermissions` to `auto` (forced anyway once v2.1.257 stopped honoring project-level `bypassPermissions`). The hook logs `decide --decision "classifier denied <cmd>"` and returns `retry: false`, so a denial becomes a journal entry the PR shows, and the attempt budget decides what happens next. |
